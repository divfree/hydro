...
template <class Scal>
class MeshStructured : public MeshGeneric<Scal, 2> {
 public:
  ...
  static constexpr size_t kCellNumNeighbourFaces = 4;
  static constexpr size_t kCellNumNeighbourNodes = 4;
  static constexpr size_t kFaceNumNeighbourNodes = 2;
  ...
  IdxCell GetNeighbourCell(IdxCell idx, size_t n) const override {
    if (fc_is_inner_[idx]) {
      ...
      MIdx offset = (std::vector<MIdx>{
        MIdx{-1, 0}, MIdx{1, 0}, MIdx{0, -1}, MIdx{0, 1}})[n];
    ...
    , ff_is_excluded_(b_faces_, false)
{
  ...
  {
    auto offset = [this](IntIdx i, IntIdx j) {
  ...
  {
    auto offset = [this](IntIdx i, IntIdx j) {
      return static_cast<IntIdx>(b_nodes_.GetIdx(MIdx(i, j)).GetRaw() -
